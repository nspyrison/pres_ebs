...
rt_anim <- animate_plotly(
  ggt142, fps = 4L, width = w * (2/3), height = w * (4/9)) %>%
  layout(dragmode = FALSE, showlegend = FALSE, 
         xaxis = list(scaleratio = 3L)) ## extra wide cause densities are narrow
```
...
- species of penguin, mapped to color & shape (later model response)
...
- Fit a __randomForest__ model predicting cluster
- Extract the SHAP values from _each_ observation using __treeshap__, Komisarczyk _et al._, 2021 
