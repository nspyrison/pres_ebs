...
rt_anim <- animate_plotly(ggt142) %>% 
  layout(dragmode = FALSE, showlegend = FALSE) %>% ## Set drag left mouse
...
- Local explanation of model & _SHAP_
...
- Intrinsic Data Dimensionality, $idd$, is the intrinsic data dimensionality
    - Usually estimated with 'fractal' methods, see 18x `Rdimtools::est.*` function, or subjectively look at correlation plots for stratification
    - _eg._ a piece of paper exists in 3D, most of the time thickness is unimportant
- Be careful when $p>idd$:
...
Defined by basis path/production:

```{r}
kableExtra::kbl(data.frame(
  tour_type = 
    c("grand", "guided", "manual", "local"), 
  target_bases = c("random bases", "objective function (simulated annealing)", 
                   "change the contribution of selected variable", "random bases within a local vacinity")))
```
...
**{spinifex} (v0.3.1), ggproto api; additive {ggplot2} construction of animated tours**
...
## Local explanation of a model & _SHAP_
...
- Processed FIFA 2020 data, RF model predicting soccer player's wage from the evaluation of their skills
    - Estimated $idd$, manually & subjectively constructed 9 aggregated variables using `{corrplot}` before modeling
...
- 420 obersvations, 4 variables (2 signal, 2 noise)
- {randomForest} model predicting cluster membership
...
cheem::run_app()
```
- Alternatively, see a hosted shiny app at: [ebsmonash.shinyapps.io/cheem_initial/](https://ebsmonash.shinyapps.io/cheem_initial/)

--
<br>
__next steps__
- Turning attention to regression case
- Language and documentation generalized to agnostic of type of model or local explanation, but functions only handle {randomForest} & {treeshap} atm.
...
- {cheem} & {spinifex}; perfromant rendering & shiny apps?
...
<br><br>
Thanks to Professor Przemyslaw Biecek for his time and input suggesting SHAP and FIFA in addition to the `{DALEX}` package eco-system and *Exploratory Model Analysis* book
