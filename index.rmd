...
- “All non-linear projections are wrong, but some are useful.”
    - --- Anatasios Panagiotelis (play on George Box’s quote about models)
...
- Often times our output space is of the same dimensionality as our input space
    - PCA returns a $pxp$ matrix, an reorientation of the axes directions ordered by the variation contained in each component.
...
if(F){
  anim <- animate_gganimate(
    ggt, options(gganimate.dev_args = list(width = 720, height = 480))
  )
...
- Control it's contribution with a manual tour
...
Also see: [A review of State-of-the-Art on Tours](https://arxiv.org/pdf/2104.08016.pdf), prebublish WIREs paper
